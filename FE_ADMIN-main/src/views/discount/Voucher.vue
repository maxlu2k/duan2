...
let vouchers = ref([]);
let actionString = ref("New voucher");
let voucher = ref({
    id: null,
    name: "TEO",
    minPrice: "",
...
const validation = () => {
    ...
    // name
    if (isNull(voucher.value.name)) {
        tmp = false;
        voucherValidate.value.name = "Please fill name";
    }
    else if (!isValid(voucher.value.name.trim().length, 1, 120)) {
        tmp = false;
...
function voucherConfirm() {
    ...
    console.log(voucher.value.maxPrice);
    console.log(voucher.value.maxPrice.length);
    lgDemo.value = false;
    if (voucher.value.id == null) {
        ...
        Axios.post("/voucher/new", voucher.value).then(function (resp) {
        ...
        Axios.put("/voucher/update", voucher.value).then(function (resp) {
            if (resp.data.infor_response.code == 200) {
...
Axios.get("/voucher/get-all").then(function (resp) {
    console.log(resp.data);
...
function edit(index) {
    actionString.value = 'Update voucher';
...
function newVC() {
    ...
    actionString.value = 'New voucher';
    lgDemo.value = true;
}

function del(index) {
    if (confirm("Are you sure delete voucher with id : " + index)) {
        Axios.delete("/voucher/delete/" + index).then(resp => {
            rs = resp.data;
            if (resp.data.infor_response.code == 200) {
